...
usage() {
	cat <<USAGE
Usage: ./tools/approve-auto-update.sh [--preview|-t] [--port PORT] [--tx-group-id ID] [--fee FEE]

Approves the latest pending AUTO_UPDATE manifest transaction.
...
tx=$(curl --silent --show-error --url "http://localhost:${port}/arbitrary/search?txGroupId=${tx_group_id}&service=AUTO_UPDATE&confirmationStatus=CONFIRMED&limit=1&reverse=true")
