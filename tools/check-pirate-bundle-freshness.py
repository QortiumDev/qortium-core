...
Exit code 0 means upstream's latest release is no newer than the greater of
the pinned tag and the acknowledged version -- i.e. nothing new to review.
Exit code 1 means upstream has moved past what Core pins and what has been
explicitly acknowledged, and lists the steps to catch up.
...
def fetch_latest_release(repo: str) -> Optional[dict]:
    """Fetch the latest release for a GitHub repo. Returns None if the repo
    has no releases (404), which is tolerated for the fork."""
...
def release_info_from_payload(payload: Optional[dict]) -> ReleaseInfo:
    if payload is None:
        return ReleaseInfo(version=None, published_at=None, url=None, present=False)
...
def format_version(parsed: Optional[ParsedVersion]) -> str:
    if parsed is None:
        return "(none)"
    return parsed.raw
...
def run_check(
    ...
    acknowledged = parse_version(ack["acknowledgedUpstreamVersion"])
    ...
    if fork_release.present and compare_base(fork_release.version, pinned) < 0:
        print(
            "WARNING: the fork's latest published release "
            f"({fork_release.version.raw}) is behind the pinned tag "
            f"({pinned.raw}). This may indicate a stale or missing fork "
            "release rather than an upstream freshness issue."
        )
        print()

    if not upstream_release.present:
        print(
            "WARNING: could not determine upstream's latest release; "
            "skipping the freshness comparison this run."
        )
        return 0

    ceiling = pinned if compare_base(pinned, acknowledged) >= 0 else acknowledged
    if compare_base(upstream_release.version, ceiling) <= 0:
        print(
            f"OK: upstream's latest release ({upstream_release.version.raw}) is "
            f"covered by the pinned tag ({pinned.raw}) and/or the acknowledged "
            f"version ({acknowledged.raw}). No action needed."
        )
        return 0

    print(f"UPSTREAM AHEAD: upstream has released {upstream_release.version.raw}, "
    ...
    print(f"     new upstream release ({upstream_release.version.raw}).")
...
def self_test() -> None:
    ...
    assert pass_code == 0, "acknowledged upstream version must not fail the check"

    # --- upstream ahead of both pin and acknowledgement must fail (exit 1). ---
    fail_code = run_check(
        pinned_tag="v1.1.9",
    ...
    assert fail_code == 1, "un-acknowledged upstream release must fail the check"

    # --- no releases on the fork must be tolerated, not raise. ---
    tolerant_code = run_check(
        ...
        fork_release=ReleaseInfo(version=None, published_at=None, url=None, present=False),
    )
    assert tolerant_code == 0

    # --- missing upstream release info must warn but not fail. ---
    unknown_upstream_code = run_check(
        pinned_tag="v1.1.9",
        ack={"acknowledgedUpstreamVersion": "v1.1.9", "note": "test"},
        upstream_release=ReleaseInfo(version=None, published_at=None, url=None, present=False),
        fork_release=ReleaseInfo(version=None, published_at=None, url=None, present=False),
    )
    assert unknown_upstream_code == 0
...
def main() -> int:
    ...
    if args.offline:
        ...
        upstream_payload = fetch_latest_release(UPSTREAM_REPO)
        fork_payload = fetch_latest_release(FORK_REPO)
