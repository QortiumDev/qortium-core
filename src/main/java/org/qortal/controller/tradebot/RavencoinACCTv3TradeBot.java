...
public class RavencoinACCTv3TradeBot implements AcctTradeBot {
	...
	/**
	 * Creates a new trade-bot entry from the "Bob" viewpoint, i.e. OFFERing NATIVE in exchange for RVN.
	 * <p>
	 * Generates:
	 * <ul>
	...
	private void handleBobWaitingForAtRedeem(Repository repository, TradeBotData tradeBotData,
		...
		if (crossChainTradeData.mode == AcctMode.REFUNDED || crossChainTradeData.mode == AcctMode.CANCELLED) {
			// Alice hasn't redeemed the native asset, so there is no point in trying to redeem the RVN
			TradeBot.updateTradeBotState(repository, tradeBotData, State.BOB_REFUNDED,
