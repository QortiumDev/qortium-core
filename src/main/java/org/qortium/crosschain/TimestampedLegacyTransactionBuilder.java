...
final class TimestampedLegacyTransactionBuilder {
	...
	private TimestampedLegacyTransactionBuilder() {
	}
	...
	static BitcoinySignedTransaction buildSpend(Bitcoiny bitcoiny, String xprv58, Map<String, Long> amountByRecipient, Long feePerByte) {
		return buildSpend(bitcoiny, xprv58, amountByRecipient, feePerByte, currentTimestamp());
	}

	static BitcoinySignedTransaction buildSpend(Bitcoiny bitcoiny, String xprv58, String recipient, long amount, Long feePerByte, long timestamp) {
		return buildSpend(bitcoiny, xprv58, Map.of(recipient, amount), feePerByte, timestamp);
	}

	static BitcoinySignedTransaction buildSpend(Bitcoiny bitcoiny, String xprv58, Map<String, Long> amountByRecipient, Long feePerByte, long timestamp) {
		try {
			...
			return buildSignedTransaction(selectedInputs, outputs, 0L, timestamp);
		} catch (ForeignBlockchainException | RuntimeException e) {
			Bitcoiny.LOGGER.warn("Unable to build timestamped legacy spend transaction: {}", e.getMessage());
			return null;
		}
	}

	static BitcoinySignedTransaction buildSpendMax(Bitcoiny bitcoiny, String xprv58, String recipient, Long feePerByte) {
		return buildSpendMax(bitcoiny, xprv58, recipient, feePerByte, currentTimestamp());
	}

	static BitcoinySignedTransaction buildSpendMax(Bitcoiny bitcoiny, String xprv58, String recipient, Long feePerByte, long timestamp) {
		try {
			List<SpendCandidate> spendCandidates = spendCandidates(bitcoiny, xprv58);
			...
			Bitcoiny.LOGGER.warn("Unable to build timestamped legacy send-max transaction: {}", e.getMessage());
			return null;
		}
	}
	...
	static BitcoinySignedTransaction buildRedeem(Bitcoiny bitcoiny, Coin amount, ECKey spendKey, List<UnspentOutput> fundingOutputs,
		...
		return buildRedeem(bitcoiny, amount, spendKey, fundingOutputs, redeemScriptBytes, secret, outputPublicKeyHash, currentTimestamp());
	}

	static BitcoinySignedTransaction buildRedeem(Bitcoiny bitcoiny, Coin amount, ECKey spendKey, List<UnspentOutput> fundingOutputs,
			byte[] redeemScriptBytes, byte[] secret, byte[] outputPublicKeyHash, long timestamp) throws ForeignBlockchainException {
		byte[] redeemPubKey = spendKey.getPubKey();
		return buildP2sh(amount.value, spendKey, fundingOutputs, redeemScriptBytes, 0L,
				NO_LOCKTIME_SEQUENCE, outputPublicKeyHash, timestamp,
	...
	static BitcoinySignedTransaction buildRefund(Bitcoiny bitcoiny, Coin amount, ECKey spendKey, List<UnspentOutput> fundingOutputs,
		...
		return buildRefund(bitcoiny, amount, spendKey, fundingOutputs, redeemScriptBytes, lockTime, outputPublicKeyHash, currentTimestamp());
	}

	static BitcoinySignedTransaction buildRefund(Bitcoiny bitcoiny, Coin amount, ECKey spendKey, List<UnspentOutput> fundingOutputs,
			byte[] redeemScriptBytes, long lockTime, byte[] outputPublicKeyHash, long timestamp) throws ForeignBlockchainException {
		byte[] refundPubKey = spendKey.getPubKey();
		return buildP2sh(amount.value, spendKey, fundingOutputs, redeemScriptBytes, lockTime,
				LOCKTIME_SEQUENCE, outputPublicKeyHash, timestamp,
				txSigBytes -> Bytes.concat(
						BitcoinyScript.pushData(txSigBytes),
	...
	private static BitcoinySignedTransaction buildP2sh(long amount, ECKey spendKey, List<UnspentOutput> fundingOutputs,
			byte[] redeemScriptBytes, long lockTime, long sequence, byte[] outputPublicKeyHash, long timestamp, ScriptSigFactory scriptSigFactory)
		...
		return buildSignedTransaction(inputs, outputs, lockTime, timestamp);
	}

	private static BitcoinySignedTransaction buildSignedTransaction(List<Input> inputs, List<Output> outputs, long lockTime, long timestamp) {
		validateUnsignedInt(timestamp, "timestamp");
		validateUnsignedInt(lockTime, "lockTime");

		for (int inputIndex = 0; inputIndex < inputs.size(); ++inputIndex) {
			Input input = inputs.get(inputIndex);
			byte[] signature = sign(input.key, signatureHash(inputs, outputs, lockTime, timestamp, inputIndex));
			input.scriptSig = input.scriptSigFactory.build(signature);
		}

		byte[] rawTransaction = serialize(inputs, outputs, lockTime, timestamp, -1);
		return BitcoinySignedTransaction.fromRaw(rawTransaction);
	}

	private static byte[] signatureHash(List<Input> inputs, List<Output> outputs, long lockTime, long timestamp, int inputIndex) {
		byte[] signaturePreimage = serialize(inputs, outputs, lockTime, timestamp, inputIndex);
	...
	private static byte[] serialize(List<Input> inputs, List<Output> outputs, long lockTime, long timestamp, int signatureInputIndex) {
		ByteArrayOutputStream stream = new ByteArrayOutputStream();
	...
	private static long currentTimestamp() {
		...
		return (now != null ? now : System.currentTimeMillis()) / 1000L;
	}
