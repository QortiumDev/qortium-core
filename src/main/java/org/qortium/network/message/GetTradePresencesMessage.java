...
public class GetTradePresencesMessage extends Message {
	...
	public static Message fromByteBuffer(int id, ByteBuffer bytes) throws MessageException {
		int groupedEntriesCount = GroupedMessageUtils.readInitialGroupCount(bytes,
				Transformer.PUBLIC_KEY_LENGTH, "invalid grouped entries count");
		...
		while (groupedEntriesCount > 0) {
			...
			if (bytes.hasRemaining()) {
				groupedEntriesCount = GroupedMessageUtils.readNextGroupCount(bytes,
						Transformer.PUBLIC_KEY_LENGTH, "invalid grouped entries count");
			} else {
				// we've finished
