...
public class HSQLDBSetGroupAvatarTransactionRepository extends HSQLDBTransactionRepository {
	...
	TransactionData fromBase(BaseTransactionData base) throws DataException {
		try (ResultSet rs = this.repository.checkedExecute("SELECT group_id, avatar_signature, group_reference FROM SetGroupAvatarTransactions WHERE signature = ?", base.getSignature())) {
			if (rs == null) return null;
			return new SetGroupAvatarTransactionData(base, rs.getInt(1), rs.getBytes(2), rs.getBytes(3));
	...
	@Override public void save(TransactionData data) throws DataException {
		...
		save.bind("signature", tx.getSignature()).bind("owner", tx.getOwnerPublicKey()).bind("group_id", tx.getGroupId())
				.bind("avatar_signature", tx.getAvatarSignature()).bind("group_reference", tx.getGroupReference());
