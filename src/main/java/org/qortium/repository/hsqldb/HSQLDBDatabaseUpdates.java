...
public class HSQLDBDatabaseUpdates {
	...
	private static void upgradeFromVersion3(Connection connection) throws SQLException {
		...
		try (Statement stmt = connection.createStatement()) {
			...
			stmt.execute("ALTER TABLE PUBLIC.\"GROUPS\" ADD COLUMN AVATAR_NAME VARCHAR(128)");
			...
			stmt.execute("CREATE TABLE PUBLIC.SETGROUPAVATARTRANSACTIONS("
					...
					+ "AVATAR_SERVICE SMALLINT, AVATAR_NAME VARCHAR(128), AVATAR_IDENTIFIER VARCHAR(64), GROUP_REFERENCE PUBLIC.SIGNATURE, "
	...
	private static void upgradeFromVersion4(Connection connection) throws SQLException {
		try (Statement stmt = connection.createStatement()) {
			stmt.execute("CREATE TABLE PUBLIC.ACCOUNTAVATARS(ACCOUNT PUBLIC.ACCOUNTADDRESS PRIMARY KEY, AVATAR_SERVICE SMALLINT NOT NULL, AVATAR_NAME VARCHAR(128) NOT NULL, AVATAR_IDENTIFIER VARCHAR(64) NOT NULL, FOREIGN KEY(ACCOUNT) REFERENCES PUBLIC.ACCOUNTS(ACCOUNT) ON DELETE CASCADE)");
			stmt.execute("CREATE TABLE PUBLIC.SETACCOUNTAVATARTRANSACTIONS(SIGNATURE PUBLIC.SIGNATURE PRIMARY KEY, OWNER PUBLIC.ACCOUNTPUBLICKEY NOT NULL, AVATAR_SERVICE SMALLINT, AVATAR_NAME VARCHAR(128), AVATAR_IDENTIFIER VARCHAR(64), PREVIOUS_AVATAR_SERVICE SMALLINT, PREVIOUS_AVATAR_NAME VARCHAR(128), PREVIOUS_AVATAR_IDENTIFIER VARCHAR(64), FOREIGN KEY(SIGNATURE) REFERENCES PUBLIC.TRANSACTIONS(SIGNATURE) ON DELETE CASCADE)");
