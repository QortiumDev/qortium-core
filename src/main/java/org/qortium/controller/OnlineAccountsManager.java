...
public class OnlineAccountsManager {
        ...
        try (final Repository repository = RepositoryManager.getRepository()) {
            ...
            for (PrivateKeyAccount onlineAccount : onlineAccounts) {
                ...
                byte[] signature = isSignatureV2Active(nextBlockHeight)
                        ? Ed25519Extras.sign(onlineAccount.getPrivateKey(), timestampBytes)
                        : Ed25519Extras.signForAggregation(onlineAccount.getPrivateKey(), timestampBytes);
    ...
    public boolean verifyOrCacheV2OnlineAccountSignature(byte[] publicKey, byte[] signature, long onlineAccountTimestamp) {
        ...
        if (!Ed25519Extras.verify(publicKey, signature, timestampBytes))
            return false;
    ...
    private static boolean isSignatureValidForHeight(OnlineAccountData onlineAccountData, int blockHeight) {
        ...
        return isSignatureV2Active(blockHeight)
                ...
                : Ed25519Extras.verifyAggregated(onlineAccountData.getPublicKey(), onlineAccountData.getSignature(), data);
    }
    ...
    private boolean computeOurAccountsForTimestamp(Long onlineAccountsTimestamp) {
        if (onlineAccountsTimestamp != null) {
            ...
            for (MintingAccountData mintingAccountData : mintingAccounts) {
                ...
                byte[] signature = isSignatureV2Active(nextBlockHeight)
                        ? Ed25519Extras.sign(privateKey, timestampBytes)
                        : Ed25519Extras.signForAggregation(privateKey, timestampBytes);
