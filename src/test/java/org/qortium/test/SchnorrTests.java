...
public class SchnorrTests extends Ed25519Extras {
    ...
    public void testSecureSignAndVerify() {
        ...
        byte[] signature = Ed25519Extras.sign(privateKey, message);

        // Honest signature verifies
        assertTrue(Ed25519Extras.verify(publicKey, signature, message));

        // Tampered message is rejected
        byte[] otherMessage = message.clone();
        otherMessage[0] ^= 0x01;
        assertFalse(Ed25519Extras.verify(publicKey, signature, otherMessage));
    }
    ...
    public void testForgeryRejectedBySecureVerify() {
        ...
        Ed25519Extras.generatePublicKey(victimPrivateKey, 0, victimPublicKey, 0);
        ...
        assertFalse("secure verify must reject the forgery", Ed25519Extras.verify(victimPublicKey, forged, message));
    }
    ...
    private static byte[] forge(byte[] publicKey, byte[] message) {
        ...
        PointAffine pA = Ed25519Extras.newPointAffine();
        ...
        PointAccum pR = Ed25519Extras.newPointAccum();
