...
The cause was that Core told every server it could speak protocol versions 1.2 through 2.0 while only implementing the calls from 1.4. Servers running the new ElectrumX 2.0 release took Core at its word and selected protocol 1.7, and at that version the `blockchain.scripthash.*` calls Core depends on no longer exist, so every wallet read against those servers came back as an unknown method. Core now negotiates 1.4 through 1.7, the range it genuinely implements, and speaks whichever of those a server picks: it remembers the version each connection settled on and chooses the matching calls for the life of that connection. At 1.7 that means the new `blockchain.scriptpubkey.*` calls, which take the payment script itself instead of a hash of it and wrap their answers in an object; Core converts those answers back into the same internal form the older calls produce, so nothing downstream can tell the difference. Protocol 1.6 changes are handled too: it removed the relay-fee call and started returning block headers as a list instead of one long concatenated string. That last one turned out to matter widely — of the 466 servers Core ships with, 320 top out at 1.6, and reading their headers with the old expectation would have failed. Protocol versions are also now compared digit group by digit group rather than as decimal numbers, so a server offering 1.10 or 1.20 is correctly seen as newer than 1.7 rather than being mistaken for 1.1 or 1.2 and wrongly accepted. The background foreign-payment notification service follows the same rules, including the detail that the 1.7 rename of the subscription call did not rename what its notifications carry.

This was verified against live servers, one per protocol generation: Digibyte and Namecoin at 1.7, Firo at 1.6, Litecoin at 1.4.2. Asking the same server for the same payment script's history twice — once forced to 1.4 and once at 1.7 — returns identical results. An opt-in acceptance test performs those checks on demand.

Separately, the bundled Electrum server list was regenerated. Firo's three servers had rotated their TLS certificates since the list was last built in July, so Core was rejecting them outright as untrusted; their pins are now current and were re-verified against the live certificates. The refresh tool can now re-pin a server whose certificate has rotated, which previously was impossible — a rotation silently dropped the server from the regenerated list, and that is how Firo, Komodo, Verus, Zcash, Verge and Peercoin lost all of their default servers on an intermediate run. Because accepting a changed pin is a fresh trust decision rather than routine maintenance, it only happens when the operator asks for it with `--accept-rotated-pins`; without the flag a rotated server is still reported and dropped, and when the flag is used every accepted rotation is written to an audit manifest recording the old and new fingerprint and when it was seen. A running node's behaviour is unchanged: it still refuses outright to connect to a server whose certificate does not match its pin.

The observable effect is that Digibyte, Namecoin and Firo wallet reads work again, and against far more servers than before. The shipped list carries 466 endpoints, up from 459: 105 kept their address but presented a new certificate and were re-pinned, 26 were added and 19 were dropped for failing verification. Two of the dropped ones are Ravencoin's `rvn-dashboard.com` and `rvn4lyfe.com`, which now refuse connections outright, taking Ravencoin from five servers to three. LBC keeps its single plaintext TCP seed, restored by hand because the refresh tool cannot verify plaintext servers, and it remains gated behind the existing opt-in setting.
...
Qortium nodes without giving those nodes a private key. The new exact public
routes return unsigned bytes only, use the normal transaction checks and
MemoryPoW-fee nonce layout, and share the existing public builder rate,
concurrency, and body-size limits. Ordinary and seed Previewnet profiles enable
only these two group write builders by default; signing remains local and the
existing invite, approval, moderation, and server-side signing routes stay
restricted. Untouched managed settings gain the routes on upgrade while an
operator's customized public route list remains preserved.
...
they never did: the repository carried two copies of the Previewnet chain
config, and the copy shipped inside the release zip (`preview/previewchain.json`)
was missing the activation trigger that had been added to the canonical copy
(`src/main/resources/previewchain.json`). Nodes load the external file from the
zip in preference to the one bundled in the jar, so every node kept paying
rewards (and counting blocks minted) one block at a time past 50,000. Because
blocks past 50,000 were already minted under per-block rules, the activation
cannot be applied retroactively; the trigger is rescheduled to height 55,000,
which all nodes must be running this config before reaching. To stop the two
copies drifting again, the duplicate under `preview/` is deleted and the
release packaging now ships the canonical `src/main/resources` file, so the
zip and the jar can never disagree. The old placeholder value inherited from
Qortal (a top-level start height of 1,508,000) is removed from the Previewnet
config at the same time, leaving the feature trigger as the single source of
truth.
