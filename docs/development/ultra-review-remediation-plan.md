...
| --- | --- | --- | --- | --- |
| T1 | Use the locally derived execution height for all three AT chain-query activation gates; add hostile claimed-height tests. | In progress | None | Pending |
| T2 | Repair mixed-peer recovery-mode exit, then contain the peer-claim watchdog by disabling it outside an explicit development profile. | Planned | T1 released to all consensus participants before its activation boundary | State-transition and minting-policy tests; watchdog default/profile tests |
...
| T5 | Make generic defaults, Docker, Previewnet profiles, ports, seed lists, and chain identities coherent. | Planned | Decide whether generic Docker targets no network or explicit Previewnet | Cross-profile and container configuration invariants |
...
| T9 | Configuration and documentation hygiene: trigger registry/testnet parity, public-write classifier invariants, Docker `.env`, and missing changelog entries. | Decision | Trigger compatibility design; T5 for Docker ownership | Config coverage, full handler-chain tests, docs reconciliation |
...
| N-02 | The recovery watchdog can orphan up to three local tip blocks based on unverified peer tip and archive claims. Peer connections are handshaked, but the asserted chain evidence is not authenticated. | High | T2/T3 | Planned | Pending |
...
| S-05 | The persistent API TLS PKCS12 keystore is written with ambient umask permissions rather than owner-only permissions. The inspected managed runtime file was mode `0660`. | Medium | T4 | Planned | Pending |
...
| D-01 | `.env.example` still maps legacy `1239x` ports while current generic Core defaults and Compose fallbacks use `1489x`. | Medium Docker | T5/T9 | Planned | Pending |
