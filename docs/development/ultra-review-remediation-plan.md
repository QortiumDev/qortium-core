...
| T7 | Repair adaptive networking: per-peer/coalesced AIMD loss, a QDN-specific catch-up signal, a bounded GET_BLOCKS budget, and one in-flight ping per peer. | In progress | T2 recovery-state semantics for the catch-up signal | Deterministic loss, peer isolation, deadline, cancellation, and ping-order tests |
...
| N-04 | GET_BLOCKS auto-degrade can spend about seven full batch timeouts on one dead peer before fallback. The loop is bounded and interruptible, but monopolizes the Synchronizer thread and delays fallback or later synchronization work. | Medium | T7 | In progress | Pending |
| N-05 | QDN yielding uses `Controller.isUpToDate()`, so a fresh node below the minting/sync peer quorum is permanently capped as if it were catching up. | Medium | T7 | In progress | Pending |
| N-06 | AIMD halves a process-global window once per expired chunk-map entry, allowing one stalled peer to collapse batching for all transfers. | High | T7 | In progress | Pending |
| N-07 | `peerPingTimeoutMillis` may exceed the fixed ping interval, permitting overlapping tasks and out-of-order consecutive-miss accounting. | Medium | T7 | In progress | Pending |
...
T7 adaptive networking is active. Its boundary is limited to per-peer and
coalesced QDN AIMD feedback, a catch-up signal that does not depend on the
minting peer quorum, a total GET_BLOCKS retry deadline with prompt cancellation,
and one in-flight ping task per peer. It may not change message schemas, chain
selection or validation, QDN authorization, archive serving, or consensus
rules. T6 is complete in Core source and local validation but is not released
or deployed. T6 scoped per-recipient QDN requests and responses, inbound
