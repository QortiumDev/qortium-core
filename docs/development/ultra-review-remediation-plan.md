...
| T9 | Configuration and documentation hygiene: strict trigger registry with activation-aware schedule compatibility, testnet parity, shared public-write classification, and missing changelog entries. | In progress | Activation-aware schedule capability approved | Config/handshake coverage, full handler-chain tests, docs reconciliation, focused/full suite, artifact inspection, independent review |
...
| C-02 | Unknown feature-trigger names are accepted and silently unused. This is currently intentional forward-compatibility behavior, and `featureTriggers` is excluded from the chain-config handshake hash. | High safety / design | T9 | In progress | Strict registered names plus a separately advertised activation schedule that remains compatible before its announced enforcement height and is required afterward; base chain hash remains stable |
...
| S-02 | Public-write protection classifies several endpoints by exact path. The reported anonymous bypass is false for shipped profiles because the preceding access handler exact-matches the same path; shared classification remains necessary defense-in-depth. | Hardening | T9 | In progress | Shared access/protection route semantics plus adversarial full handler-chain coverage |
...
| T-01 | The manual local-testnet profile omits the current feature-trigger schedule and therefore exercises different consensus behavior from Previewnet. | Medium test fidelity | T9 | In progress | Explicit intentional test heights for every registered Preview trigger plus parity coverage |
...
| D-02 | The three adaptive-networking merges lack their required human-readable changelog entries. | Low documentation | T9 | In progress | Backfill exact-title entries for PRs #212, #213, and #214 |
...
T9 configuration and documentation hygiene is active. Its boundary is limited
to a strict feature-trigger registry, a separately advertised activation-aware
schedule compatibility contract that leaves the existing base chain hash
unchanged, explicit local-testnet trigger decisions, shared public-write route
classification, and the three missing adaptive-networking changelog entries.
It may not change trigger heights in Previewnet, transaction/block validation,
public endpoint authorization, anti-abuse limits, chain selection, or consensus
behavior at any already configured height.
