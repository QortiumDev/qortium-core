...
No implementation tranche is active. T7 is complete in Core source, tests, and
the local packaged artifact, but it is not released or deployed; T8 repository
and API correctness is the next planned boundary. T7 changed no message schema,
chain selection or validation rule, QDN authorization, archive serving rule, or
consensus behavior.
