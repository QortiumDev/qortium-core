...
`ForeignBlockchainRegistry` is the central lookup point for crosschain runtime code. It resolves canonical names and currency codes, maps Bitcoiny foreign-chain ids back to registered chains, owns the runtime instance definitions, exposes registry iteration helpers and the shared ACCT registry, and keeps `SupportedBlockchain` as a transitional facade for older API/query plumbing. New backend code should use the registry directly instead of adding new `SupportedBlockchain` dependencies.
